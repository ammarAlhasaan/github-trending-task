...
import {
  Dropdown, ListItem, ListItemLoader, PageTitle, PageWrapper,
} from '../../../components';
...
export const Developers = () => {
  ...
  const [developers, setRepositories] = useState([]);
  ...
  const getRepos = (q: string = '') => {
    api.get(`/developers${q}`).then((response: any) => {
      setError(false);
      const { data } = response;
  ...
  const items = developers?.length === 0 ? (<h3>Not Found, Please Try Again</h3>)
    : developers.map((repo: any, index) => (
      <ItemDeveloper
        key={String(index)}
        {...repo}
      />
